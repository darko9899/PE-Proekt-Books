...
void showOrders();
int validDate(string dateForCheck);
...
void addBook() {
    ...
    for(int i=0;i<n;i++){
     ...
     while(basd==0){
            ...
            basd=validDate(dateOfPublication);
                if(basd==0 )cout<<"Wrong format of date!!!"<<endl;
    }
...
vector<Books> printBooks(){
    ...
    while(1){
        ...
        gek >> price;
                ...
                Books *b=new Books(title,*a,edition,ISBN,dateOfPublication,quantity,dateOfApproval,price);
...
void makeOrder(){
            ...
            Orders *order=new Orders(bs,orederedBooks,totalPrice);
